...
function getApiToken(): string {
  return process.env.FINMIND_API_TOKEN || process.env.FINANCIAL_DATASETS_API_KEY || '';
}

/**
 * Shared request execution: handles API token, error handling, logging, and response parsing.
 */
async function executeRequest(
  ...
  if (!token) {
    logger.warn(`[FinMind API] call without token: ${label}`);
  }
